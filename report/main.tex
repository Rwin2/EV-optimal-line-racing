...
Finding the optimal racing line on a sinuous closed track is a classical problem in motorsport engineering. On straight sections, every driver can floor it and reach top speed, so there is little room for differentiation. But in the corners, performance differences emerge since the choice of racing line governs the maximum speed a car can carry through a turn, and small improvements in cornering translate into significant lap time gains \citep{Xiong2010}. Beyond the geometric question of which path to follow, the vehicle dynamics (tire grip limits, weight transfer, drift behavior) make this a rich nonlinear optimization problem.

We add an additional layer of complexity by considering electric vehicles (EVs). Unlike conventional race cars that can burn fuel freely, an EV must continuously manage its battery state of charge ($SOC$). Aggressive acceleration and high cornering speeds drain the battery faster through increased motor power demand and aerodynamic losses. Conversely, conservative driving preserves energy but sacrifices lap time. This creates a genuine dual-objective problem as we want to go fast \textit{and} be energy-efficient. In the context of Formula E and the growing EV racing scene, this tradeoff is not just academic because it directly determines race strategy.

The objective of this project is to develop an optimization-based controller that computes racing trajectories for EVs on arbitrary closed circuits, jointly minimizing lap time and energy consumption. We consider a vehicle navigating a known 2D track and seek a control policy that determines steering, acceleration, and braking over time while respecting vehicle dynamics, tire grip constraints, track boundaries, and battery limits. This is inherently a constrained multi-objective optimization problem, combining trajectory planning (the spatial path) with speed profiling (how fast to drive along that path).
...
We use a planar bicycle model, which captures both longitudinal and lateral dynamics while remaining tractable for real-time optimization. The state vector is $\mathbf{z} = [x,\, y,\, \psi,\, v_x,\, v_y,\, \omega,\, SOC]^\top$, where $(x, y)$ is position, $\psi$ is heading angle, $(v_x, v_y)$ are longitudinal and lateral velocities, $\omega$ is yaw rate, and $SOC \in [0, 1]$ is battery state of charge. The control inputs are $\mathbf{u} = [\delta,\, F_{\text{drive}}]^\top$, that is steering angle and net longitudinal force (positive for traction, negative for regenerative braking).
